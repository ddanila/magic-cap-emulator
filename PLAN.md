...
| TX39 core | `MADD`/`MADDU` plus three-operand `MULT`/`MULTU`; all eight integer and eight coprocessor branch-likely forms, `SYNC`, and the `SDBBP`/`DERET` self-debug path with NMI, Debug/DEPC, `NIS`/`OES`, and single-step suppression; one-cycle multiply and load dependent-GPR interlocks including the unaligned-load target bypass; independent 35-cycle divide unit with HI/LO interlocks and cancellation; Config Halt/Doze, cache and processor-rate semantics; direct-mapped instruction cache plus two-way, write-through/no-write-allocate data cache with LRU, auto-lock, locked stores, lock clearing, and Dino DMA invalidation; Config-selected refill and processor-clock divisors. Injected code plus the monitor's checksummed discovery verify arithmetic, branch nullification, debug/exception state, pipeline timing, masked physical-interrupt wake, CP0, replacement, lock, refill, DMA coherency, and 1/2/4/8 clock-rate contracts | [`tx39-cpu.md`](docs/tx39-cpu.md) |
