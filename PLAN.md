...
| Persistence & power | Battery-backed DRAM + RTC as NVRAM; power-button suspend/wake across a retained-RAM relaunch; Privacy sets and confirms a synthetic PIN, requests it on every wake, rejects a wrong PIN and unlocks with the correct one | [`power-wake.md`](docs/power-wake.md) |
...
| TX39 core | `MADD`/`MADDU` plus three-operand `MULT`/`MULTU`; all eight integer and eight coprocessor branch-likely forms, `SYNC`, R3900 no-op TLB encodings and reserved coprocessor-load/store priority; the `SDBBP`/`DERET` self-debug path with NMI, Debug/DEPC, `NIS`/`OES`, and single-step suppression; one-cycle multiply and load dependent-GPR interlocks including the unaligned-load target bypass; independent 35-cycle divide unit with HI/LO interlocks and cancellation; Config Halt/Doze, cache and processor-rate semantics; all four manual-defined `CACHE` operations with CP0 privilege enforcement; direct-mapped instruction cache plus two-way, write-through/no-write-allocate data cache with LRU, auto-lock, locked stores, lock clearing, and Dino DMA invalidation; Config-selected refill and processor-clock divisors. Injected code plus the monitor's checksummed discovery verify arithmetic, branch nullification, debug/exception state, pipeline timing, masked physical-interrupt wake, CP0 privilege, replacement, lock, refill, DMA coherency, and 1/2/4/8 clock-rate contracts | [`tx39-cpu.md`](docs/tx39-cpu.md) |
...
- **Model the built-in modem's external line side.** The SIB telecom DMA ring
  ...
  segments without the disconnect warning. A second answer-side mode now
  persists asynchronous PPP framing across every ROM read and passes complete
  IPv4 datagrams to a source-built libslirp adapter. The layer-2 shim supplies
  ARP internally, trims Ethernet padding, clamps the host TCP window to the
  4,096-byte value accepted by the period-era stack, and returns exact IPv4
  datagrams through `SoftModemWrite`. It supports arbitrary TCP/UDP flows and
  multiple sequential or simultaneous connections rather than the explicit
  16 KiB response contract. A product-level acceptance follows a `302` from
  `/` to `/second` across two TCP connections, exchanges fourteen packets in
  each direction, and OCRs `Magic Cap bridge works twice.`. Carrier
  acquisition remains timing sensitive, so the harness has bounded pre-IP
  watchdogs and can retry from one immutable retained state with `--attempts`.
  This closes the unrestricted multi-request bridge target; the working PC
...
- **Improve hardware fidelity beyond observed ROM needs.** TX39 Config now
  ...
  Bus, SIB and periodic-timer engines and restarts them without discarding
  their programmed state. The periodic timer exposes its live 16-bit
  countdown and preserves partial intervals across its dedicated freeze and
  master gate. The power-control stop timer now counts exact 128 Hz intervals
  independently of `masterClock`, matching the monitor's three nominal 16 ms
  (15.625 ms) Betty-reset phases and the OS's nominal 64 ms (62.5 ms)
  DeepDoze refresh wake. The monitor-observed RTC test modes now advance the
  high byte and low 32-bit stage independently, and the ROM's real `SetTimer`
  returns success at its requested value. TX39 multiply and multiply/add now
  issue every cycle, stall an immediately dependent GPR reader for one cycle,
  and expose HI/LO without that stall. Its 35-cycle divider runs independently,
  ...
  entering an ordinary exception, for both loads and stores. Config Halt and
  Doze now stall the pipeline, preserve divider progress, distinguish Halt
