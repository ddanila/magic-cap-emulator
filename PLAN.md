...
| Built-in software modem | Continuous 48-word SIB telecom DMA ring drives the ROM's V.32 pump/control/FIR through a TX39 `MADD`; the Betty DAA hookswitch/line input and Dino MFIO ring edges are modelled; the deterministic exchange supplies 350+440 Hz dial tone and decodes outbound DTMF | [`builtin-modem.md`](docs/builtin-modem.md) |
...
- **Model the built-in modem's external line side.** The SIB telecom DMA ring
  ...
  tone-dialed numbers. Product-level dialing through the normal UI/actor path,
  pulse digits, carrier acquisition and a remote modem are not yet represented.
  Fax is the final documented product behavior beyond that boundary. This is
