...
| TX39 core | `MADD`/`MADDU` plus three-operand `MULT`/`MULTU`; all eight integer branch-likely forms and `SYNC`; one-cycle multiply and load dependent-GPR interlocks including the unaligned-load target bypass; independent 35-cycle divide unit with HI/LO interlocks and cancellation; Config and Cache CP0 semantics; direct-mapped instruction cache plus two-way, write-through/no-write-allocate data cache with LRU, auto-lock, locked stores, lock clearing, and Dino DMA invalidation; Config-selected refill and processor-clock divisors. Injected code plus the monitor's checksummed discovery verify arithmetic, branch nullification, pipeline timing, CP0, replacement, lock, refill, DMA coherency, and 1/2/4/8 clock-rate contracts | [`tx39-cpu.md`](docs/tx39-cpu.md) |
...
- **Improve hardware fidelity beyond observed ROM needs.** TX39 Config now
  ...
  such opcode occurs in a sized SDK ELF function.
