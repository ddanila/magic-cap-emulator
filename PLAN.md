...
- **Model the built-in modem's external line side.** The SIB telecom DMA ring
  ...
  its own request with the assigned address and VJ options. The remaining
  line-side target is completing IPCP and bridging IP to a host network
  endpoint. This is separate from the working PC Card PPP path
