...
| TX39 core | `MADD`/`MADDU` plus three-operand `MULT`/`MULTU`; Config cache-size, enable and write-lock semantics; Cache auto-lock mode stacking across exceptions/RFE. Injected code verifies all four arithmetic results and both CP0 contracts | [`tx39-cpu.md`](docs/tx39-cpu.md) |
...
- **Improve hardware fidelity beyond observed ROM needs.** TX39 Config now
  ...
  auto-lock modes now stack on exception and RFE. Per-line two-way cache
  replacement/locking, refill bursts, reduced-frequency clock timing, and many
