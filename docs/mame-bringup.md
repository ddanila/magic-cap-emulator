...
actors, the connection monitor, PPP output, answer-side ROM-queue echo,
product-side PPP input and LCP-frame processing. Artifacts remain under
...
still incomplete. The current gaps include supplying a remote PPP peer and
host-network bridge for the now-connected built-in dial-up path, multi-device
