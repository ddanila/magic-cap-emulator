...
| Telephone and built-in modem | The built-in fax modem uses a telephone line, presents receive-fax/answer choices for an incoming call, and supports tone/pulse configuration (pp. 94–95, 135–163, 216) | ROM V.32 and fax code plus continuous telecom DMA execute; Betty connected/off-hook and the Dino ring detector pass; one held ring envelope qualifies through the ROM, notifies PhoneServer/FaxReceive, and opens Phone Status; **receive fax** starts live-call `AnswerModem`, both fax HDLC directions, the Receiving fax window and non-silent PCM; direct 350+440 Hz dial-tone capture and DTMF/pulse-dialed `580` pass; a full-duplex PCM bridge exchanges every telecom word between two independent DataRovers | Internal ROM/DSP, digital DAA, incoming-call/fax-answer UI, exchange dialing, normal Telephone tone-dial path and remote PCM transport covered; matched fax origin, carrier/data and completed fax transfer are missing |
...
1. **Built-in line side.** Drive a matched product fax originator against the
   now-working **receive fax** side over the PCM bridge, acquire carrier and
   complete a page transfer. The incoming call object, local fax answer/DSP
   startup, normal Telephone's digit-dependent analog output, and remote
   transport are now covered.
