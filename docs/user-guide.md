...
| Telephone and built-in modem | The built-in fax modem uses a telephone line and supports tone/pulse configuration (pp. 135–163, 216) | ROM V.32 code and continuous telecom DMA execute; Betty connected/off-hook, Dino ring edges, direct 350+440 Hz dial-tone capture, and DTMF and pulse-dialed `580` decoding pass; the visible Telephone's real generated output is decoded as `580`; a full-duplex PCM bridge exchanges every telecom word between two independent DataRovers | Internal ROM/DSP, digital DAA, exchange dialing, normal Telephone tone-dial path and remote PCM transport covered; paired-modem carrier, data and fax are missing |
...
1. **Built-in line side.** Drive the Internet Center through carrier
   acquisition between the two DataRover softmodems now connected by the PCM
   bridge, exchange data and only then extend to fax. The normal Telephone's
   digit-dependent analog output and the remote transport are now covered.
