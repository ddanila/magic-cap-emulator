...
| Telephone and built-in modem | The built-in fax modem uses a telephone line, presents receive-fax/answer choices for an incoming call, and supports tone/pulse configuration (pp. 94–95, 135–163, 216) | ROM V.32 code and continuous telecom DMA execute; Betty connected/off-hook and the Dino ring detector pass; one held ring envelope qualifies through the ROM, notifies PhoneServer/FaxReceive, and opens Phone Status; direct 350+440 Hz dial-tone capture and DTMF/pulse-dialed `580` pass; the visible Telephone's real generated output is decoded as `580`; a full-duplex PCM bridge exchanges every telecom word between two independent DataRovers | Internal ROM/DSP, digital DAA, incoming-call UI, exchange dialing, normal Telephone tone-dial path and remote PCM transport covered; paired-modem carrier, data and completed fax transfer are missing |
...
1. **Built-in line side.** Select **receive fax** in the now-working incoming
   Phone Status window while the peer originates over the PCM bridge, acquire
   carrier, exchange data and complete a fax transfer. The incoming call
   object, normal Telephone's digit-dependent analog output, and remote
