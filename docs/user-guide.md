...
| Telephone and built-in modem | The built-in fax modem uses a telephone line and supports tone/pulse configuration (pp. 135–163, 216) | ROM V.32 code and continuous telecom DMA execute; Betty connected/off-hook, Dino ring edges, direct 350+440 Hz dial-tone capture, and DTMF and pulse-dialed `580` decoding pass | Internal ROM/DSP, digital DAA and exchange dialing boundaries covered; normal UI dialing, carrier, remote peer and fax are missing |
...
1. **Built-in line side.** Drive the real Telephone/Internet Center setup,
   cover tone and pulse dialing, provide analog tone/carrier behavior, connect
   a peer and only then extend to fax.
