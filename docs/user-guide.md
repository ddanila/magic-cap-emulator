...
| Speaker and controls | Volume and system sounds are configurable; a sound stamp can record, stop and play microphone audio (pp. 23, 51–53, 67–68) | Startup beep and continuously buffered sound-TX DMA pass; Controls → Sound moves the volume to both clamps and makes the same effect audible at maximum and silent at off; direct sound-RX DMA captures tone/silence; the real Stamper UI records, stops, drains its SIB command, and plays the captured audio into a WAV | Volume and principal sound-stamp workflow covered; reassigning each individual system sound is not a separate test |
...
2. **Smaller UI contracts.** Add a focused check for individual system-sound
   reassignment. Controls-initiated touch realignment, volume clamps,
   password-on-wake and beaming a Notebook page are covered.
