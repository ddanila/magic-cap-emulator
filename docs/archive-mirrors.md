# Preserved Magic Cap web archives

Two locally mirrored sites preserve substantially more of the Magic Cap
developer ecosystem than the small set of binaries needed to run the
DataRover. This page records what was inspected, what it adds to the emulator,
and which material is only historical context. The mirrors, extracted
copyrighted sources, packages, and PDFs remain outside Git.

## Snapshot and provenance

The snapshot inspected on 2026-07-27 is:

| Origin | Persistent local root | On-disk size / files | Principal contents |
|---|---|---:|---|
| [Josh Carter's Magic Cap archive](https://joshcarter.com/magic_cap/) | `$HOME/fun/_mirrors/joshcarter.com` | 114 MiB / 234 | DataRover ROMs and packages, eight SDK PDFs, device histories and photographs |
| [Resurrected datarover.com](http://www.datarover.com/) | `$HOME/fun/_mirrors/www.datarover.com` | 40 MiB / 1,147 | General Magic developer reference, FAQs, samples, Magic Internet Kit, tools and historical material |

The Josh Carter copy was made by HTTrack on 2026-07-26 and its log reports a
complete 227-link crawl with no errors or warnings. The datarover.com tree is
a separate local preservation copy. Origin links below are preferred when
they remain available; the local trees preserve the exact inputs actually
examined.
...
files and two bare `.sit` files, with no failures. The result contains 1,447
files (30,148,908 bytes) under:

```text
$HOME/fun/magic-cap-mirror-analysis-20260727/unpacked/
```

That persistent analysis directory is intentional; `/tmp` is too ephemeral
for repeatable reverse engineering. To inspect another archive without
changing the mirror:

```sh
analysis_root="$HOME/fun/magic-cap-mirror-analysis-20260727"
mkdir -p "$analysis_root/unpacked"
unar -o "$analysis_root/unpacked" \
  "$HOME/fun/_mirrors/www.datarover.com/path/to/archive.sit.hqx"
```
...
|---|---:|---|---|
| *Design and Magic Cap* | 78 | `3cdcbf165ee8b5c9d69de28cdc476f6fb34851cb29d52de80da55a7c2181d4c3` | OS and UI design rationale; behavioral context, not registers |
| *Guide to Development Tools* | 114 | `ff3a0e2ad59f51c053d892c332d930d1eadab4cd0354e3a35f015d91f34e93e0` | MIPS package build, monitor download and debugger workflow |
| *Magic Internet Kit* | 26 | `68e0f8542e4b0d3f7f71e46b33a8041b41b3baebf8aa4235d8f05f8a2e974a44` | TCP, DNS, PPP and communications application API |
| *Package Development Guide* | 114 | `61ecc7048d335f340aae04dec2f8a68e4aaade2f25b46e198bce0f23e557b742` | Clusters, shadows, storage-card objects and package lifecycle |
| *Magic Developer Roadmap* | 8 | `a6fa4c4027154591bf0363229a6f9099950123aa5474fa8e86dff2285ff5143d` | SDK orientation |
| *Magic Cap Tutorial* | 60 | `16f51903ce7c8ec09ef30df838b253636c97139f092a682ebb2f1c53252f9311` | Source-level sample and package acceptance material |
| *Magic Cap User Interface Specification* | 324 | `c56d1eafd00b38972bfb1c503b65eea3cf60bfbfd648339c4f8c65183de184ad` | Detailed UI oracle for future workflow automation |
| *Using Magic Cap* | 234 | `20010cefe051b94fde9f8fa16273a6c33547e85cc061fb5e80625296fa21f22a` | Exact duplicate of the already adopted DataRover guide |
...
The datarover.com developer tree also contains 167 API-reference pages, 32
concept pages, 29 FAQ pages, six CodeWarrior/Magic Developer tool chapters,
22 standard sample archives, and 138 daily Magic Cap list digests from
May–October 1996. The reference and concept collections are an offline map of
the portable OS API. The FAQs and list are particularly valuable for edge
cases, but mailing-list reports should be corroborated before becoming
hardware behavior.
...
5. Install `Translation.pkg`, insert a preserved 1.x fixture, and separately
...
The extracted `Random Code/DownloadPackage` sample documents a different,
...
Consequently, the working PCLink harness is correct to send the preserved raw
...
- `WCPack.pkg` bundles AirSurfer, PM100C CDPD and Ricochet drivers;
- `WaveLAN.pkg` supports the Lucent WaveLAN Bronze 802.11 card, without
  encryption or SSID selection;
- `AirSurfer.pkg` supports NetWave wireless Ethernet;
- `PM100C.pkg` and `Ricochet.pkg` are wireless modem/ISP paths;
- `Ne2000.pkg` supports Socket LP and SohoWare ND5100 wireline cards;
- `EtherLinkIII.pkg`, built 1998-09-20, is the already adopted 3C589 driver;
- `Translation.pkg` migrates selected Magic Cap 1.x card data into 3.1; and
- browser 3.5 is a useful stock baseline because its own text explicitly
...
The extracted Mitsubishi M37690 material describes a dedicated Magic Bus
peripheral MCU rather than the DataRover's controller:
...
Several smaller sources are useful as tests:
...
- The memory-runtime documentation explains metaclusters, clusters, shadows,
  ...
  investigate user-visible cleanup/error symptoms, but no mirrored source
...
- `DownloadROMImage` and `DownloadAndNubs` preserve corrected classic-Mac
  ...
  the later Apollo SDK and preserved `WinDownload` already provide the
  relevant DataRover download path; these archives add no register behavior.

`OrigEnvoy15equates.sit.hqx` contains a 553,610-byte Motorola Envoy 1.5 ROM
equate file. `PIC2000Scripts.sit.hqx` contains a 549,228-byte PIC-2000 system
equate file and target/debugger scripts. These are valuable if a public ROM
for either 68K platform is recovered, but they do not fill any DataRover
Apollo register gap. No public Envoy or Sony ROM dump was found in the
mirrors.
...
| Artifact | SHA-256 |
|---|---|
| `MIK.zip` | `0e910646116be761c2f02bbb858ee5064801dcbaefa42dd394cf8cc4ece8f3fe` |
| `MagicInternetKit.sit.hqx` | `46b02586cbc2719277b1423e498949e86267b6b188889e9116d9811324eb8b6b` |
| `RandomCode.sit.hqx` | `d37b28b82075ef3a0df810fcc8b15f7b55f200618854173073a49341e96911f5` |
| `ThirdPartyOffers.sit.hqx` | `8f846c7a551382ede08df68e6317bed974f35e478f1247d7f229e1a24d05a789` |
| `cookbook_samples.sit.hqx` | `ca4b7776bdaba8588b5471a394d192facebbbf3b1e6313dbe558a10528c17e9a` |
| `OrigEnvoy15equates.sit.hqx` | `7b17b10a1ad2a517ad53e329e279560cde336cd19d2d26d71d5e79fa3075261b` |
| `PIC2000Scripts.sit.hqx` | `8d7aabb3cb59946a61585a35afa578d189b8294ce6123e02c096255b30108309` |
| `Translation.pkg` | `4f3be2b81e5868e70b321df8473e98fccdcb3ca90bc1c8999cc823490bd32cd0` |
| `WCPack.pkg` | `82dbb1b75d3456df583fb216504932084e804de0bc583df4a587469b209ce74a` |
| `WaveLAN.pkg` | `57c26768a1762e474d6129b43bf5bdc8fade2c3e2483392f28a48c6f52921732` |
| `AirSurfer.pkg` | `258c8899b4e3fd86728b3642ec08a062454bb5a0596deca47faa0756d56291c8` |
| `PM100C.pkg` | `3ed7bceaa1d1618c837c5ed50497d0f777c4128d9ce8ffe6850f29ffd1fcc964` |
| `Ricochet.pkg` | `d71fe857b7ba3c26658f88f3539309decc758720359292244c40d1fe89c6669f` |
| `Ne2000.pkg` | `bbedb30c71f99bd64d9c982b940b7346a488cb7aeee900503e3208ed7a290fe5` |

## Effect on the roadmap

The snapshot changes priorities without claiming new implemented behavior:
